...
- **Web**：React + Vite，支持在纸面上**拖放开窗**、拖动移位、填写 x/y/宽/高，
  并可选择 **1 / 5 / 10 毫米定位步长**，三种编辑方式按同一规则即时吸附
...
定位步长（保存/恢复、偏离刻度 422、旧格式按 1 毫米）、web → /api 反代接线。
...
- 任一字段非法（非 JSON 整数、宽高 < 1、越过安全区、偏离步长刻度、缺字段、
  编号超长/重复、step 不是 1/5/10 等）→ **422**，
  响应按开窗行号给出**逐字段错误**（编号错误的字段名为 `label`），且**整次提交不落库**。
- 合法（即使裁决为“不可裁切”）→ **200**，保存步长、全部开窗与完整裁决：

```json
{
  "verdict": "不可裁切",
  "step": 5,
  "windows": [{ "id": 11, "position": 0, "x": 252, "y": 172, "w": 60, "h": 40, "label": "ZW-2026-001" }],
  "result": {
    "verdict": "不可裁切",
...
│   ├── app/engine.py       # 裁决引擎（半开矩形、逐字段校验、瑕疵区常量、步长刻度）
│   ├── app/validation.py   # 原始 JSON 整数校验（拒绝 3.5 / "5" / true）、编号与步长校验
│   ├── app/migrate.py      # 启动时的幂等轻量迁移（为旧库补 windows.label / layouts.step 可空列）
│   └── tests/              # pytest
└── frontend/               # React + Vite
    ├── src/geometry.js     # 与引擎同构的前端镜像（即时预览与步长吸附，后端为准）
